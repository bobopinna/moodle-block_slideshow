YUI.add('moodle-block_slideshow-slideshow', function(Y) {
	...
	*/
	...
	Y.extend(Slideshow, Y.Widget, {
		...
		*/
		slide: function($which){
			...
			this.set('_transition', this.get('transition'));
			...
			switch( $which ){
				...
				case 'previous':
					...
					switch (this.get('transition')) {
						case Slideshow.PRESETS.slideLeft:
							this.set('_transition', Slideshow.PRESETS.slideRight);
						break;
						case Slideshow.PRESETS.slideRight:
							this.set('_transition', Slideshow.PRESETS.slideLeft);
						break;
						case Slideshow.PRESETS.slideUp:
							this.set('_transition', Slideshow.PRESETS.slideDown);
						break;
						case Slideshow.PRESETS.slideDown:
							this.set('_transition', Slideshow.PRESETS.slideUp);
						break;
					}
					break;	
				...
				default:
					...
					if( $which == $currentIndex ){
						...
						if(parseInt($which, 10) < $currentIndex) {
							switch (this.get('transition')) {
								case Slideshow.PRESETS.slideLeft:
									this.set('_transition', Slideshow.PRESETS.slideRight);
								break;
		...
		*/
		_handlePageClick: function(e){
			...
			if ( ! e.target.hasClass( this.getClassName('page') )) {
		...
		*/
		_slide: function(e){
			...
			var $that				= this,
				$slideTransition	= this.get('_transition'),
		...
		*/
		initializer: function(){
			...
			this.get('contentBox').get('children').setStyles({'position': 'absolute', 'display': 'block', 'visibility': 'hidden'});
	...
	function getMaxHeight(node, maxheight){
		if (!node.get('children').isEmpty()) {
			node.get('children').each(function (childnode) {
				maxheight = Math.max(getMaxHeight(childnode, maxheight), maxheight);
			});
		}

		return Math.max(node.get('offsetHeight'), maxheight);
	}
